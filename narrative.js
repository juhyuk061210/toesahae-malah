...
(function (global) {
  ...
  function answersBlock(answersLabel, report) {
    ...
    L.push("퇴사 생각 빈도: " + (a.quitFrequency || "-"));
    L.push("가장 큰 이유: " + (a.quitReason || "-"));
    L.push("현재 근속: " + (a.tenure || "-"));
    L.push("나가면 하고 싶은 것: " + (a.nextPlan || "-"));
    L.push("실제 준비 상태: " + (a.preparation || "-"));
    L.push("가장 알고 싶은 것: " + (a.mainQuestion || "-"));
    L.push("본인이 쓴 고민: " + (a.freeText ? '"' + a.freeText + '"' : "(미입력)"));
  ...
  function ruleChapters(read, analyze, answersLabel, report, tension) {
    ...
    function para(r, connective) {
      ...
      var s = (connective ? connective + " " : "") + head + " " + r.detail;
      if (r.evidence) s += " (" + r.evidence + ")";
      if (r.confidence === "LOW") s += " 다만 이 부분은 해석이 갈리는 자리라 단정하기는 어렵습니다.";
      return s;
    }

    /* 문단이 다 똑같이 시작하지 않게 연결어를 돌려 쓴다 */
    var LINKS = [null, "그리고", "여기에 더해", "한편", "또 하나 눈에 띄는 것은", "마지막으로"];
    ...
    function fill(id, targetChars) {
      ...
      for (var i = 0; i < rs.length; i++) {
        ...
        var t = para(rs[i], LINKS[out.length % LINKS.length]);
    ...
    EXTRA.which = function () {
      ...
      if (cs) {
        ...
        if (rows.length) {
          out.push("항목별로 나눠 보면 " + iga(rows[0].label) + " " + rows[0].value + "점으로 가장 높습니다. " +
                   rows.slice(1).map(function (x) { return x.label + " " + x.value + "점"; }).join(", ") +
                   "과 비교하면 차이가 분명합니다. " +
                   pick([
      ...
      if (A.quitReason) {
        out.push("본인이 꼽은 이유는 “" + A.quitReason + "”였습니다. " +
                 "이것이 위 판정과 같은 방향이면 원인이 분명한 것이고, 다르면 " +
                 "겉으로 드러난 이유 아래에 다른 원인이 하나 더 있다는 신호입니다. " +
                 "어느 쪽인지는 본인이 가장 잘 압니다.");
      }
      return out;
    };
      ...
      if (pf) {
        ...
        if (rows.length) {
          out.push("세 갈래를 점수로 비교하면 " +
                   ...
                   "그 형태에서 이 사람의 기질이 가장 덜 억눌린다는 뜻입니다.");
        }
      }
      out.push(pick([
    ...
    EXTRA.first = function () {
      ...
      var prepLabel = A.preparation || "지금 상태";
      out.push(pick([
        "지금 준비 상태가 “" + prepLabel + "”이라면, 다음 한 칸은 " + step + "입니다. " +
        "큰 결정을 내리는 것보다 이 한 칸을 채우는 쪽이 지금은 훨씬 도움이 됩니다.",

        "“" + prepLabel + "”이라고 답하셨으니, 바로 다음에 놓을 것은 " + step + "입니다. " +
        "결론을 먼저 내려고 하면 오히려 오래 걸립니다. 한 칸씩이 빠릅니다.",

        "다음 한 걸음만 말씀드리면 " + step + "입니다. “" + prepLabel + "” 상태에서 " +
        "그다음 칸이 바로 이것이고, 여기까지는 결정을 미룬 채로도 할 수 있습니다.",

        "오늘 할 수 있는 건 하나입니다. " + step + ". " +
        "“" + prepLabel + "”에서 다음 칸으로 넘어가는 데 필요한 건 결심이 아니라 이 한 번입니다."
      ]));
      if (y && y.용신오행) {
    ...
    EXTRA.hard = function () {
      ...
      if (pb) {
        ...
        if (rows[0].value - rows[rows.length - 1].value < 15) {
          out.push("다만 네 항목의 차이가 크지 않습니다. 특정 하나가 원인이라기보다 " +
                   "전반적으로 소모되고 있는 상태에 가깝습니다. 이런 경우 한 가지를 고쳐도 " +
                   "체감이 크게 달라지지 않아서, 환경 자체를 바꾸는 쪽을 검토하게 됩니다.");
        } else {
          out.push("차이가 뚜렷하다는 것은 손댈 지점이 분명하다는 뜻이기도 합니다. " +
                   rows[0].label + " 하나가 해결되면 나머지 체감도 함께 내려갈 가능성이 있습니다. " +
                   "퇴사를 결정하기 전에 이 항목만 따로 떼어 조정할 여지가 있는지 확인해볼 만합니다.");
        }
      }
      return out;
    };
    ...
    EXTRA.repeat = function () {
      ...
      if (g.flaws && g.flaws.length) {
        out.push("타고난 구조에서 반복적으로 걸리는 자리가 있습니다. " +
                 ...
                 "이것은 결함이 아니라 이 사람이 같은 상황에서 같은 방식으로 반응한다는 뜻입니다. " +
