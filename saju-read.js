...
(function (global) {
  ...
  function readGyukguk(a) {
    ...
    if (g.flaws.length) {
      ...
      out.push({
        ...
        detail: "이건 결함이 아니라 이 사람이 반복해서 부딪히는 자리입니다. " +
  ...
  function readRelations(a) {
    ...
    top.forEach(function (r) {
      ...
      if (r.type === "육충") {
        out.push({
          ...
          detail: "충은 깨지는 게 아니라 열리는 것이기도 하다. " +
        ...
        out.push({
          ...
          detail: (r.mean || "믿었던 쪽과 어긋나거나, 좋게 시작한 관계가 끝이 안 좋아지는 형태다.") +
                  " 충처럼 한 번에 터지지 않고 오래 끈다.",
    ...
    if (rel.note) {
      var NOTE = {
        탐합망충: "움직일 조건은 있는데 붙잡는 힘이 더 세다 — 결정이 늦어진다",
        충중봉합: "붙잡는 게 있어도 움직임이 이긴다 — 변화가 실제로 일어난다",
        합충병존: "묶였다 풀렸다를 반복한다 — 결심과 번복이 잦다"
      };
      out.push({
  ...
  var SIN_READ = {
    ...
    양인: { claim:"결정할 때 세게 밀어붙이는 성향",
            detail:"미루다가 한 번에 결단합니다. 그래서 퇴사도 갑자기 나올 수 있다 — 속도 조절이 필요합니다." },
    ...
    망신: { claim:"드러나는 것에 민감한 자리", detail:"체면이 판단에 영향을 준다 — 그걸 알고 결정하면 덜 흔들립니다." },
  ...
  function readSinsal(a) {
    ...
    (a.sinsal.top || []).forEach(function (s) {
      ...
      out.push({
        ...
        claim: r.claim,
        detail: r.detail + (s.damped ? " 다만 충·공망이 겹쳐 이 성향이 늘 일정하게 나오지는 않습니다." : ""),
  ...
  function readStrength(a) {
    ...
    if (st.borderline) {
      out.push({
        ...
        detail: "밀어주는 힘과 덜어내는 힘이 비슷하다. 그래서 '이런 사람'이라고 " +
                "한마디로 규정되지 않는다. 환경이 성향을 만듭니다.",
    ...
    out.push({
      ...
      claim: weak
        ...
        : "스스로 밀고 나가는 힘이 강한 타입",
      detail: weak
        ? "체력이나 의지의 문제가 아니라, 지원·팀·시스템이 있을 때 성과가 확 달라지는 구조입니다. 혼자 하는 일에서 소진이 빠르다."
        : "지시받는 것보다 맡겨지는 쪽이 편하다. 다만 도움을 안 구해서 혼자 짊어지는 일이 잦다.",
