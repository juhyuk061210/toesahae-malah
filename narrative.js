...
(function (global) {
  ...
  function ruleChapters(read, analyze, answersLabel, report, tension) {
    ...
    EXTRA.path = function () {
      ...
      /* 실측 48%가 250자 미만이었다. 세 갈래를 비교하는 장인데
         일반론 한 문단으로 끝나면 손님은 "그래서 뭐"라고 느낀다.
         용신·조후·유통은 "어느 형태가 맞나"에 직접 답하는 판정이다. */
      var PATHSAY = {
        유통_식상생재: "만든 것이 값으로 바로 돌아오는 구조라, 성과와 보상이 멀리 떨어진 형태에서는 " +
                       "동기가 잘 안 붙습니다. 세 갈래 중에서는 성과가 바로 보이는 쪽이 결이 맞습니다.",
        유통_관인상생: "조직 안에서 평가가 힘으로 돌아오는 구조입니다. " +
                       "혼자 하는 형태로 가면 그 회로가 끊겨서, 실력이 있어도 속도가 안 붙습니다.",
        유통_재생관: "숫자로 증명하면 자리가 따라오는 구조입니다. " +
                     "실적이 그대로 드러나는 형태라면 어느 갈래든 맞고, 가려지는 형태면 어디서든 답답합니다.",
        유통_상관패인: "배우면서 하는 형태에서 힘이 가장 잘 나갑니다. " +
                       "학습이 끊기는 자리로 가면 그 힘이 마찰로 바뀝니다.",
        유통_식신제살: "압박을 결과물로 바꾸는 구조라 강도 높은 형태를 견딥니다. " +
                       "다만 총량 관리가 안 되는 형태는 어느 갈래든 오래 못 갑니다.",
        유통_단절: "지금은 어느 갈래를 고르든 통로가 좁은 상태입니다. " +
                   "형태를 고르기 전에 힘이 나갈 길을 하나 만드는 쪽이 순서입니다."
      };
      var JOHUSAY = {
        조후_결핍: "타고난 명식에 계절이 요구하는 것이 없습니다. 그래서 형태보다 " +
                   "그게 채워지는 자리인지가 훨씬 크게 작용합니다. 혼자 하는 쪽으로 갈수록 " +
                   "그걸 스스로 만들어야 해서 부담이 커집니다.",
        조후_충족: "필요한 조건을 이미 갖고 계셔서 형태 선택의 폭이 넓은 편입니다. " +
                   "세 갈래 중 어디든 초기 적응은 무난한 쪽입니다."
      };
      (read.readings || []).forEach(function (rr) {
        if (PATHSAY[rr.tag]) out.push(PATHSAY[rr.tag]);
        if (JOHUSAY[rr.tag]) out.push(JOHUSAY[rr.tag]);
      });
      if (pf) {
    ...
    EXTRA.repeat = function () {
      ...
      /* 이 장은 상품이 파는 핵심("옮겨도 따라오는 것")인데,
         재료가 네 개뿐이라 하나만 비면 63자짜리 장이 나갔다.
         실측 36%가 250자 미만이었다.
         관성 상태·유통 구조·궁위는 전부 "회사를 바꿔도 남는 것"에 직결되는
         판정인데 이 장에서 안 쓰이고 있었다. 억지로 채우는 게 아니라
         원래 여기 있어야 할 재료를 가져온다. */
      var CARRY = {
        관성합거: "직함은 오는데 결정권이 옆으로 새는 배치는 회사가 바뀌어도 잘 안 바뀝니다. " +
                  "다음 자리를 볼 때 '누가 결정하는가'를 조건표보다 먼저 확인하셔야 합니다.",
        관성공망: "자리를 얻어도 금방 허전해지는 감각은 조직을 옮긴다고 채워지지 않습니다. " +
                  "직함이 아니라 무엇이 남는 일인지를 기준으로 고르시는 편이 낫습니다.",
        관성손상: "위가 바뀌고 기준이 바뀌는 일을 자주 겪는 배치입니다. " +
                  "안정돼 보이는 조직으로 옮겨도 몇 년 안에 비슷한 장면이 옵니다. " +
                  "그럴 때 흔들리지 않을 자기 기준을 만들어두는 쪽이 실질적입니다.",
        관성불안정: "소속을 원하는 마음과 그 소속이 안 버텨주는 현실은 사람에 붙은 조건입니다. " +
                    "회사를 바꾸면 잠깐 가라앉았다가 같은 자리에서 다시 올라옵니다.",
        관성미약: "촘촘한 위계에서 빨리 지치는 성질은 옮겨도 그대로입니다. " +
                  "다음 자리를 볼 때 연봉보다 재량의 폭을 먼저 보셔야 합니다.",
        비소속형: "직장이 정체성이 되지 않는 배치라, 어느 회사에 가도 '여기가 내 자리다'는 감각은 " +
                  "잘 안 생깁니다. 그건 회사 문제가 아니라 연결되는 방식의 차이입니다.",
        조직잠재형: "안에서는 버티는데 밖에서 안 보이는 배치입니다. " +
                    "옮겨도 이 구도가 그대로면 인정이 늦어지는 것도 그대로 반복됩니다.",
        월령소속형: "회사가 하루의 축이 되어 있는 배치라, 옮기는 것보다 " +
                    "하루가 어떻게 짜이는지가 만족을 더 크게 좌우합니다.",
        조직소속형: "직함과 소속이 자기 자리를 지탱하는 배치입니다. " +
                    "그래서 그만두는 순간 일보다 '내가 누구인지'가 먼저 흔들립니다. " +
                    "이건 다음 회사에서도 똑같이 작동하니, 옮길 때 형식이 있는 자리인지를 보셔야 합니다.",
        관성과다: "거절하면 관계가 깨진다는 감각이 기본값으로 돌아가는 배치입니다. " +
                  "이건 업무량 문제가 아니라 반응 방식이라, 새 조직에서도 어느새 짐이 몰립니다.",
        조후_결핍: "이 계절에 필요한 것이 명식에 없어서 밖에서 끌어와야 하는 배치입니다. " +
                   "회사를 옮겨도 그게 없는 자리면 같은 헤맴이 반복됩니다. " +
                   "조건표보다 그게 있는 자리인지가 더 크게 좌우합니다."
      };
      var FLOWCARRY = {
        유통_단절: "힘이 한 군데 고여 흐르지 않는 구도는 자리를 바꾼다고 풀리지 않습니다. " +
                   "통로를 만드는 쪽이 먼저이고, 그게 없으면 어느 회사에서도 같은 답답함이 옵니다."
      };
      (read.readings || []).forEach(function (rr) {
        if (CARRY[rr.tag]) out.push(CARRY[rr.tag]);
        if (FLOWCARRY[rr.tag]) out.push(FLOWCARRY[rr.tag]);
        if (rr.tag && rr.tag.indexOf("충_") === 0 && rr.tag !== "충_배경") {
          out.push(rr.claim + "입니다. " + rr.detail +
                   " 이건 특정 회사의 사정이 아니라 명식에 걸린 자리라, 옮겨도 함께 갑니다.");
        }
      });
    ...
    CHAPTERS.forEach(function (c) {
      ...
      /* 소제목과 본문 첫 문장이 글자까지 같은 경우가 80장 중 29장이었다.
         ...
         그 claim 이 소제목으로도 쓰이기 때문이다.
         손님은 같은 말을 두 줄 연달아 읽게 된다.
         겹치면 본문 쪽 머리말을 떼고 바로 설명으로 들어간다. */
      if (paras.length) {
        var stem = String(lead).replace(/[.!?]+$/, "");
        var head0 = String(paras[0]);
        if (stem && head0.indexOf(stem) === 0) {
          var rest = head0.slice(stem.length).replace(/^(입니다|이다)?[.!?]*\s*/, "");
          if (rest.length > 40) paras[0] = rest;
        }
      }
    ...
    return {
      /* 헤드라인이 어느 장의 문단과 통째로 겹치면 손님은 같은 문장을
         맨 위에서 한 번, 본문에서 또 한 번 읽는다. 실측 17.8%였다.
         본문을 지우면 그 장이 얇아지므로 헤드라인 쪽을 다른 판정에서 고른다. */
      headline: (function () {
        /* 문단 머리 12자로 비교한다. 짧은 claim 은 뒤에 "입니다"가 붙어
           길이가 달라지므로 뒷부분까지 맞춰 보면 못 잡는다. */
        var used = {};
        out.forEach(function (c) {
          (c.body || []).forEach(function (t) { used[String(t).slice(0, 12)] = 1; });
          if (c.lead) used[String(c.lead).slice(0, 12)] = 1;
        });
        var rs = read.readings || [];
        for (var i = 0; i < rs.length; i++) {
          var cl = String(rs[i].claim || "");
          if (cl.length >= 12 && !used[cl.slice(0, 12)]) return cl;
        }
        return rs.length ? rs[0].claim : "지금의 흐름을 정리해드립니다";
      })(),
      chapters: out,
