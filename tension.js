...
(function (global) {
  ...
  */
  ...
  },
  {
    ...
    say: function (c) {
      return {
        ...
        body: "공고는 보셨는데, 마지막으로 실제로 뭘 해본 때를 물었더니 ‘" +
              L("lastAction", c.a.lastAction) + "’ 라고 하셨습니다. 그사이에도 퇴사 생각은 " +
              L("quitFrequency", c.a.quitFrequency) + " 나셨고요.",
  ...
  },
  {
    ...
    say: function (c) {
      return {
        ...
        body: "소득이 끊겨도 ‘" + L("runway", c.a.runway) + "’ 라고 답하셨습니다. " +
  ...
  },
  {
    ...
    say: function (c) {
      return {
        ...
        /* under1 라벨은 "1개월도 못 버틴다" 라 뒤에 "버틸 수 있다고" 를 붙이면
           뜻이 정반대가 된다. 라벨을 그대로 인용하는 형태로 바꾼다. */
        body: "이번 달 안에 나가실 생각이라고 답하셨습니다. 그런데 소득이 끊겼을 때는 " +
              "‘" + L("runway", c.a.runway) + "’ 라고 답하셨습니다. " +
              "이 두 답은 서로 다른 이야기를 하고 있습니다.",
    ...
    say: function (c) {
      return {
        ...
        body: "'잘 버텼다 싶은 게 솔직히 없다'고 답하셨습니다. " +
              ...
              "‘" + L("exception", c.a.exception) + "’ 라고 하셨고요. " +
              "1년 동안 아무것도 없었을 리는 없습니다.",
        cushion: null
      };
    }
  },
    ...
    say: function (c) {
      return {
        ...
        body: "퇴사 생각은 " + L("quitFrequency", c.a.quitFrequency) + "인데, " +
              "‘그래도 이건 할 만하다’고 느낀 때로는 ‘" +
              L("exception", c.a.exception) + "’ 을 고르셨습니다.",
    ...
    say: function () {
      return {
        ...
        body: "이 리포트는 그 판단을 대신할 수 없습니다. " +
              ...
              "건강 문제로 다루는 게 맞습니다.",
        cushion: "혼자 정리하기 어려우실 때 통화할 수 있는 곳이 있습니다 — " +
                 "정신건강 상담전화 1577-0199, 자살예방 상담전화 109 (24시간, 무료)."
      };
    },
  ...
  /* ─────────────────────────────────────────────────────────
     R32 — 결제 전 답변만으로 걸리는 안전 분기

     R31 은 필요한 네 항목 중 셋이 결제 후 문항이고 전부 건너뛸 수 있다.
     그래서 무료 단계에서는 한 번도 발동하지 않았다.
     실측: 소진으로 그만둔 적 있고 · 준비 전무 · 한 달도 못 버팀 ·
     매일 퇴사 생각인 손님 중 안전 문구를 받은 사람이 0명이었다.
     그 사람들이 받은 문장은 "회사를 옮겨도 같은 자리에서 같은 감정이
     올라옵니다" 였다.

     여기서 말하는 건 진단이 아니다. 이런 답이 겹치면 결정을 미루고
     회복을 먼저 보자는 것, 그리고 도움받을 곳이 있다는 것뿐이다.
     ───────────────────────────────────────────────────────── */
  {
    id: "R36", grade: "safety", rate: 3, group: "safety",
    needs: ["endured", "selfLabel", "quitFrequency"],
    when: function (c) {
      /* 지난 1년 잘 버틴 것이 하나도 없다 + 원인을 자신에게 돌린다 +
         거의 매일 퇴사 생각. 여기까지가 기본 조건이다. */
      var base = c.a.endured === "none" &&
                 c.a.selfLabel === "myFault" &&
                 c.a.quitFrequency === "daily";
      if (!base) return false;
      /* 여기에 소진 이력이나 버틸 여유 없음이 하나라도 겹치면 발동한다.
         셋만으로는 흔한 상태일 수 있어 한 겹을 더 요구한다. */
      return c.a.pastQuit === "burnout" ||
             c.a.runway === "under1" ||
             c.a.body === "sleep" ||
             c.activeN === 0 && c.a.deadline === "thisMonth";
    },
    say: function () {
      return {
        claim: "지금은 나갈지 남을지를 정하기 전에 회복이 먼저로 보입니다",
        body: "지난 1년 동안 잘 버틴 것으로 꼽을 게 없다고 하셨고, " +
              "원인은 본인에게 있다고 보고 계시고, 퇴사 생각은 거의 매일 나신다고 하셨습니다. " +
              "이 세 가지가 함께 나올 때는 회사를 고르는 문제보다 " +
              "지금 상태를 회복하는 쪽이 먼저인 경우가 많습니다. " +
              "이 리포트는 그 판단을 대신할 수 없습니다.",
        cushion: "잠이나 몸에 실제로 영향이 오고 있다면 커리어가 아니라 건강 문제로 다루는 게 맞습니다. " +
                 "혼자 정리하기 어려우실 때 통화할 수 있는 곳이 있습니다 — " +
                 "정신건강 상담전화 1577-0199, 자살예방 상담전화 109 (24시간, 무료)."
      };
    },
    overrideAll: true
